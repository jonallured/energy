import { getArtworkEmailTemplate, useMailComposer } from "app/screens/Artwork/useMailComposer"
...
jest.mock("app/system/store/GlobalStore")
...
describe("useMailComposer", () => {
  ...
  describe("sendMail", () => {
    it("should call MailComposer with correct params when sending a single artwork", () => {
      ...
      const composeAsyncMock = jest.fn()
      jest.spyOn(MailComposer, "composeAsync").mockImplementation(composeAsyncMock)

      useMailComposer().sendMail({ artworks } as any)

      expect(composeAsyncMock).toHaveBeenCalledWith({
        isHtml: true,
        ccRecipients,
        subject,
        body,
    ...
    it("should call alertOnEmailFailure when there is an error sending the email", async () => {
      ...
      jest.spyOn(MailComposer, "composeAsync").mockRejectedValue(error)
